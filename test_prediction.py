...
def test_prediction(train_txs, test_txs, window=5, args=None):
    ...
    n_tx = test_simulator.max_epochs*epoch_tx_count
    test_hgraph = GraphStack(pd.DataFrame({'block':np.repeat(np.arange(test_simulator.max_epochs,dtype=int),epoch_tx_count),'from':test_txs['from'].values[:n_tx],'to':test_txs['to'].values[:n_tx]}), debug=True)
    test_simulator.reset(ptx=(window-1)*epoch_tx_count)
    mse_list = []
    baseline_mse_list = []
    real_mse_list = []
    for epoch in tqdm(range(test_simulator.max_epochs-window+1)):
        ...
        parts = Partition(graph_path).partition(n_shards)
        account_table = {a:s for a,s in zip(graph.vertex_idx,parts)}
        test_simulator.step(account_table)
